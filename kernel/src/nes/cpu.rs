...
impl NESCPU {
    ...
    pub fn clock(&mut self, cartridge: &mut Cartridge) -> NESResult<u32> {
        {
    ...
    pub fn push_stack(&mut self, data: u8, cartridge: &mut Cartridge) -> NESResult<()> {
    ...
    pub fn pop_stack(&mut self, cartridge: &mut Cartridge) -> NESResult<u8> {
        self.reg_sp = self.reg_sp.wrapping_add(1);
        let addr = 0x0100 | self.reg_sp as u16;
        CPUBus::read(addr, cartridge)
    }

    pub fn execute(&mut self, cartridge: &mut Cartridge) -> NESResult<u32> {
